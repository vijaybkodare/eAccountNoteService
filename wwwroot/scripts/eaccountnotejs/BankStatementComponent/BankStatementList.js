var BankStatementList = React.createClass({
    displayName: "BankStatementList",
    ...
    render: function () {
        ...
        return React.createElement(
            ...
            React.createElement(ListHeader, { ShowNextComponent: this.props.ShowNextComponent, Title: "Bank Statement" }),
            React.createElement(
                "div",
    ...
    getList: function () {
        return this.state.Items.map(function (item) {
            if (item.Remark.toLowerCase().indexOf(this.state.Filter) > -1 || this.state.Filter == "") {
                return React.createElement(BankStatementRow, {
                    key: item.BankStatementHeaderId,
                    Item: item,
    ...
    actionOnItemSelect: function (item) {
        this.props.ShowRep(item.BankStatementHeaderId);
    }
});