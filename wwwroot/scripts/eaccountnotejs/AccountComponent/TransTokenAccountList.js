var TransTokenAccountList = React.createClass({
    displayName: "TransTokenAccountList",
    ...
    getList: function () {
        ...
        return this.state.Items.map(function (accountDto) {
            if (accountDto.accountMaster.AccountName.toLowerCase().indexOf(this.state.Filter.toLowerCase()) > -1 || this.state.Filter == "") {
                srNo += 1;
    ...
    getListRow: function (accountDto, srNo) {
        return React.createElement(AccountTransTokenRow, { key: accountDto.accountMaster.AccountId,
