...
function getVal(id) {
    ...
    if (input != null) return input.value;else return "";
}
...
function setComponent(comp) {
    comp.show = function (arg, itemSelMode, showNextComponent, multiSelect) {
        ...
        if (comp.customShow != null) comp.customShow(arg);
    ...
    comp.hide = function (arg) {
        ...
        if (comp.customHide != null) comp.customHide(arg);
        comp.toggleFlag = false;
    };
    comp.toggle = function () {
        if (comp.toggleFlag) comp.hide();else comp.show();
    };
    comp.hide();
}
...
function readCookie(key) {
    ...
    for (var i = 0; i < cookiearray.length; i++) {
        ...
        if (keyVal[0].trim() == key) return keyVal[1].trim();
    }
    return "";
}
...
function isStringFilter(src, dst) {
    ...
    if (dst == "") return true;
    if (src.toLowerCase().indexOf(dst.toLowerCase()) >= 0) return true;else return false;
}
...
function isValidForFilter(mainFilter, detailFilter, accountName, balance, tagName) {
    ...
    if (mainFilter == "" && detailFilter && !detailFilter.CR.IsValid && !detailFilter.DR.IsValid) // && !detailFilter.TagName.IsValid)
        return true;
    if (mainFilter != "") if (accountName.toLowerCase().indexOf(mainFilter.toLowerCase()) >= 0) flag = true;
